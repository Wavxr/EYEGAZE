import { Calibrator, CalibrationMatrix, euclideanDistance } from './calibration.js';
...
export default class EyeGestures{ // strip export default before making cdn/web embeddable version 
    ...
    processKeyPoints(left_eye_coordinates,right_eye_coordinates,offset_x,offset_y,scale_x,scale_y,width,height)
    {
        ...
        const smoothingFactor = 0.1; // Adjust for desired smoothness (0.1 = slow, 0.5 = fast)
        this.smoothedPoint = this.smoothedPoint || [point[0], point[1]];
        this.smoothedPoint = [
            this.smoothedPoint[0] + smoothingFactor * (point[0] - this.smoothedPoint[0]),
            this.smoothedPoint[1] + smoothingFactor * (point[1] - this.smoothedPoint[1])
        ];
        ...
        document.getElementById("cursor-x").textContent = point[0].toFixed(2);
        document.getElementById("cursor-y").textContent = point[1].toFixed(2);
